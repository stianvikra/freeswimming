...
test.describe("my library workout builder", () => {
  test.describe("mobile density", () => {
    test.skip(
      ({ browserName, isMobile }) => isSiteLockEnabled || browserName !== "chromium" || !isMobile,
      "Workout builder mobile density e2e runs once on mobile Chromium."
    );
    ...
    test("reclaims mobile width and keeps secondary builder actions behind progressive disclosure on phone widths", async ({
      ...
      await expect(
        page.getByTestId("session-draft-repeat-mobile-add-repeat-after-2")
      ).toBeVisible();
      await expect(page.getByTestId("session-draft-repeat-mobile-remove-2")).toBeVisible();
    });
  });

  test.describe("desktop save flows", () => {
    test.skip(
      ({ browserName, isMobile }) => isSiteLockEnabled || browserName !== "chromium" || isMobile,
      "Workout builder save-flow e2e runs once on desktop Chromium."
    );
