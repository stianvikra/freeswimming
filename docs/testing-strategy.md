...
- Current ratchet baseline: CSS transfer default budget tightened from `160kb` to `150kb` on `2026-06-25`; wait for two new weekly green runs before another step.
...
- Previous tighten decision: on `2026-06-19`, `npm run test:perf:trend` recommended `tighten` (`10` weekly green runs, `17.8%` worst margin), so JS transfer default moved from `390kb` to `380kb` with another conservative `10kb` step.
- Latest audit decision: on `2026-06-25`, `npm run test:perf:trend` recommended `tighten` (`11` weekly green runs, `15.4%` worst margin). JS transfer was too close for another `10kb` step while preserving `15%` headroom, so CSS transfer default moved from `160kb` to `150kb`.
