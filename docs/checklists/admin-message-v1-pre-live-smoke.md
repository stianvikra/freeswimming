...
| ---------------- | ------------ | ---------------- | ------------------------- | ---------------------- | -------------------- | ----------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-07 07:58 | `preview`    | Codex automation | `smtp_one_com_compatible` | present                | present              | present           | pass   | Preview contact, goals coaching, and preview notify submissions returned `200`; admin API/browser smoke showed rows, redacted diagnostics, delivery attempts, and reversible status workflow; duplicate owner mailbox receipt/reply was not repeated after Production proved the real mailbox path.      |
...
- Preview and Production logs showed Upstash rate limiting returning `401`, so the app fell back to in-memory limiting during smoke.
- This does not change the smoke result because intake storage, SMTP provider acceptance, admin diagnostics, and the Production e-mail-first reply path passed.
- Repair `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` in the secrets/config governance workstream before broader public launch or higher-volume intake.
- Do not record Upstash URL/token values in checklist, PR, screenshots, logs, or docs.
