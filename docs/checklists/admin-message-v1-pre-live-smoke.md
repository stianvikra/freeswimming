...
| 2026-05-07 08:14 | `production` | Codex automation | `smtp_one_com_compatible` | present                | present              | present           | partial | Production redeploy `dpl_7vwxhecjct57GmKcD1Ad13hhQ1SA` is `READY`; contact, goals coaching, and preview notify returned `200`, admin rows show `accepted_by_provider`, reversible status workflow passed, and owner confirmed One.com receipt for a real Production message; owner reply confirmation still pending. |
