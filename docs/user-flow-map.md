...
- `/my-library/habits`: private `My Perfect Day` habit tracker. `Add habit` opens a focused create surface in view on mobile and near the top of the Habits surface on desktop; after `Create habit`, the new row receives focus and shows `Habit added` inside the card. The route supports `date=YYYY-MM-DD` for selected-day Habits history and keeps `view=active` compatible with mobile routine entrypoints. Week controls move by 7-day windows, Today returns to the current day, URL/back-forward changes refresh the visible selected day/week, the selected week shows ISO week number/year plus date range, and the week strip shows weekday/date labels with the selected day marked. Past selected dates allow correcting eligible existing habit check-ins for that date, while `Add habit`, row `Edit`, and `Archive` stay Today-only so historical correction does not accidentally change habit setup. Future dates are not selectable through the UI and invalid/future route dates load Today. Add/edit separates cadence period, any-day frequency, and fixed weekdays; rows show labels such as `Daily`, `Weekly - any day`, or `Weekly - 2 fixed days`. User-facing modes are `Do`, `Quit`, and `Timed` while storage keeps the legacy-safe `build`, `quit`, `timed` values. `Do` target setup separates `Done only` / `Any amount` reminders from specific count, duration, time-of-day, and avoid/limit targets; existing specific count units include `times`, `steps`, `pages`, `glasses`, and `custom`, while persisted `litres` requires a future database constraint migration before it can be offered safely. Open count rows use one visible `0-100` value stepper plus `Save`; opening `Details` does not duplicate the same value editor. Desktop habit pills sit after the heading where space allows, and peer row controls keep one visual height so `Save` and `Details` do not overlap. The pre-completion binary action is `Mark done`, completed rows show green `Done` / `Done today` status, row-level `Edit` lives inside `Details` on Today, and `Rest day` in details records a non-quit habit as not done, not missed, and excluded from the selected day's perfect-day denominator. `Quit` habits keep `Log slip` in details and, after a slip, show consistency such as `9/10 days clear` plus current streak instead of only resetting to zero days. `Timed` habits show one daily total, restore same-day local timer progress after browser/PWA restart without writing server completion automatically, add saved duration plus local timer time in the visible total, let `Manual time` / `Add manual time` add external minutes, and pause the current timer when another timed habit starts; historical timed corrections use `Manual time` rather than starting a past local timer. Weekly/monthly target-met rows stay `Done this week/month` for the rest of the period. Open habits sort by nearest deadline: today/daily first, then `This week`, then `This month`, before low-interruption status, rest-day, and completed groups. Future shared My Library calendar views should reuse the same source/filter direction for `Habits`, `Micro Sessions`, `Dryland`, `Swimming`, and `All`; week/month/year comparison is planned in `docs/task-briefs/planned/2026-06-04-aw-006-my-library-calendar-period-comparison-10-10.md`, and work/off-work belongs to future context filters, not a separate calendar.
