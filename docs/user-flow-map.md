...
- `/my-library/habits`: private `My Perfect Day` habit tracker. `Add habit` opens a focused create surface in view on mobile and near the top of the Habits surface on desktop; after `Create habit`, the new row receives focus and shows `Habit added` inside the card. In mobile active view, `Week` reveals the existing 7-day overview without requiring landscape orientation; selecting prior weeks, swiping week history, and editing prior days remain future history/backfill scope. Add/edit separates cadence period, any-day frequency, and fixed weekdays; rows show labels such as `Daily`, `Weekly - any day`, or `Weekly - 2 fixed days`. User-facing modes are `Do`, `Quit`, and `Timed` while storage keeps the legacy-safe `build`, `quit`, `timed` values. `Do` habits support no-quantity `Done only` check-ins plus count/time variants; simple reminders such as morning water are best modeled as `Done only` until a future builder target semantics slice adds explicit `Any amount`, litres, or glasses options. The pre-completion binary action is `Mark done`, completed rows show green `Done` / `Done today` status, row-level `Edit` lives inside `Details`, and `Rest day` in details records a non-quit habit as not done, not missed, and excluded from today's perfect-day denominator. `Quit` habits keep `Log slip` in details and, after a slip, show consistency such as `9/10 days clear` plus current streak instead of only resetting to zero days. `Timed` habits show one daily total, restore same-day local timer progress after browser/PWA restart without writing server completion automatically, add saved duration plus local timer time in the visible total, let `Manual time` / `Add manual time` add external minutes, and pause the current timer when another timed habit starts. Weekly/monthly target-met rows stay `Done this week/month` for the rest of the period. Open habits sort by nearest deadline: today/daily first, then `This week`, then `This month`, before low-interruption status, rest-day, and completed groups.
