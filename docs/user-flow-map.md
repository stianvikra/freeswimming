...
- `/my-library/habits`: private `My Perfect Day` habit tracker. Add/edit separates cadence period, any-day frequency, and fixed weekdays; rows show labels such as `Daily`, `Weekly - any day`, or `Weekly - 2 fixed days`. Active rows are grouped by due `Build`, due `Timed`, `Quit status`, `Done today` / `Done this week`, then later/not-due rows. `Build` habits support no-quantity `Done only` check-ins plus count/time variants, row-level `Edit` lives inside `Details`, `Quit` habits keep `Log slip` in details, and `Timed` habits show daily timer progress such as `0:00 / 8:00 today` while summary cards remain 7-day rollups.
