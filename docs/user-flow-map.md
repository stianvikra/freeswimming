...
- `/my-library/habits`: private `My Perfect Day` habit tracker. `Add habit` opens a focused create surface in view on mobile and near the top of the Habits surface on desktop; after `Create habit`, the new row receives focus and shows `Habit added` inside the card. The route supports `date=YYYY-MM-DD` for selected-day Habits history and keeps `view=active` compatible with mobile routine entrypoints. Week controls move by ISO Monday-Sunday weeks, Today returns to the current day, URL/back-forward changes refresh the visible selected day/week, date/week navigation does not add `#today-habits` or jump away from the weekly overview, the selected week shows ISO week number/year plus date range, and the week strip shows weekday/date labels; selected state only reflects loaded data, while requested dates show pending/failure feedback during load or fallback. The Habits heading area shows selected-date context text such as `Today · Jun 6` or `Sat · Jun 6` on the same row as a truthful status line such as `Today: 6/7 · Week: 1/2 · Month: 1/3`, with `Week` and `Month` shown only when active habits exist for those cadence periods. Past selected dates allow correcting eligible existing habit check-ins for that date, while `Add habit`, row `Edit this habit`, `End habit and move to Past habits`, and `Restore habit` stay Today-only so historical correction does not accidentally change habit setup. Future dates are not selectable through the UI; upcoming days in the current week are shown as disabled, and invalid/future route dates load Today.
  ...
  When Today has at least two eligible missed habit days and existing tracking history, Habits can show a compact recovery assistant such as `5 missed days` above the list. It never writes automatic failures. Each affected habit card starts collapsed with the missed-day count and next day; `Clean up missed days` / `Clean up` opens one active missed day at a time. The active day can be marked `Done` when that specific habit/day is a manual done-only habit, marked `Rest day`, left missed without writing rows, or opened with `Open day` for value, timed, source-backed, and quit-slip corrections on the exact historical day. Cards in cleanup mode show the Today completion action as `Done today` so it is distinct from missed-day recovery. Failed writes keep the same day in review. `Restart stats` is secondary and confirmation-gated; confirming creates one server-canonical reset boundary per active habit and leaves earlier check-ins visible through Calendar Comparison.
