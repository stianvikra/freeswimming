...
- `/my-library/habits`: private `My Perfect Day` habit tracker. Add/edit separates cadence period, any-day frequency, and fixed weekdays; rows show labels such as `Daily`, `3x/week any days`, or `2 fixed days/week`. Active rows are grouped by due `Build`, due `Timed`, `Quit status`, then later/not-due rows. `Build` habits use quick `Done`/`Undo` check-ins, `Quit` habits keep `Log slip` in details, and `Timed` habits show daily timer progress such as `0:00 / 8:00 today` while summary cards remain 7-day rollups.
