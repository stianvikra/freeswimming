...
| Admin notes/content managers        | `components/admin/AdminNotesManager.tsx`, `components/admin/AdminContextNotesPanel.tsx`, `components/admin/AdminContentManager.tsx`                                                                                                      | loading, warning, errors, recovery warnings, upload retry, empty, no matches                     | Dense, high-value operator workflows with staged uploads, related records, and recovery behavior. Admin Notes top-level, Context Notes, and Content Manager top-level/revision/course-workspace/inline feedback parity are done; course-structure feedback parity is the active bounded pass. | Continue deferring Content Manager recovery workflow states unless explicitly scoped.                                                  |
...
## Completed Admin Content Manager Inline Feedback State Parity Slice
...
`docs/task-briefs/done/2026-05-21-aw-006-admin-content-manager-inline-form-feedback-state-parity-10-10.md`

Completed scope:

- Reuse the existing admin-local `AdminManagerState` helper on selected `AdminContentManager` inline form feedback states:
  - workspace lesson create error,
  - edit-form dirty warning,
  - edit-form save error,
  - create-form setup/schema warning.
- Preserve copy, fetch behavior, retry callbacks, create/update payloads, dirty-state detection, course-structure recovery messages, Context Notes, Context QR, labels, and support procedures.
- Add focused component/unit coverage around inline form feedback semantics.
- Use `after/reference` screenshot handoff because rendered admin UI changes.

## Current Candidate Status

Current AW-006 state-primitive implementation candidate:

`Admin Content Manager course-structure feedback state parity`

Active implementation brief:

`docs/task-briefs/in-progress/2026-05-22-aw-006-admin-content-manager-course-structure-feedback-state-parity-10-10.md`

Scope direction:

- Reuse the existing admin-local `AdminManagerState` helper for the remaining `courseStructureMessage` rendering in `AdminContentManager`.
