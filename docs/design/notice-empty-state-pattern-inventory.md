...
| Admin notes/content managers        | `components/admin/AdminNotesManager.tsx`, `components/admin/AdminContextNotesPanel.tsx`, `components/admin/AdminContentManager.tsx`                                                                                                      | loading, warning, errors, recovery warnings, upload retry, empty, no matches                     | Dense, high-value operator workflows with staged uploads, related records, and recovery behavior. Admin Notes top-level and Context Notes state parity are done.    | No current state-primitive candidate is selected; continue deferring Content Manager recovery states until a content-manager-specific slice. |
...
No current state-primitive implementation candidate is selected after PR `#786/#787`. Run a fresh queue/design/code re-audit before selecting another AW-006 UI slice, and continue deferring `AdminContentManager` recovery states unless a content-manager-specific cleanup is explicitly scoped.
