...
| Program Builder route and exports              | `app/my-library/programs/[programId]/page.tsx`, `components/my-library/programs/ProgramBuilderHub.tsx`                                                                                                                                                                                                                                                                                  | route shell/header/back action, top create/status actions, schema warning, load error, missing scheduled workouts, save error/success, no-loaded-program guidance, recent-program reopen, Garmin-ready JSON details/download, Program PDF open status, download/open errors                               | PR `#826` closed the bounded export gap by adding accessible pending/success/error feedback for Program Builder JSON/PDF export actions while preserving program data, export routes, artifact formats, filenames, auth, persistence, and planner behavior. PR `#862` closed the route-level feedback gap. Fresh `2026-05-28` re-audit found `/my-library/programs/[programId]` and the top `ProgramBuilderHub` create/empty/recent-program actions still using older route-local rounded blue-card/action styling while adjacent My Library workspaces now use the newer AW-006 token/action hierarchy.                             | In progress: `docs/task-briefs/in-progress/2026-05-28-aw-006-program-builder-workspace-token-action-parity-10-10.md`; preserve program data, APIs, create/save/reset, week/day assignments, PDF/Garmin exports, analytics, Help/Guide, and support behavior.                           |
