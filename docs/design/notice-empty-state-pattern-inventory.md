...
| Admin notes/content managers        | `components/admin/AdminNotesManager.tsx`, `components/admin/AdminContextNotesPanel.tsx`, `components/admin/AdminContentManager.tsx`                                                                                                      | loading, warning, errors, recovery warnings, upload retry, empty, no matches                     | Dense, high-value operator workflows with staged uploads, related records, and recovery behavior. Admin Notes top-level, Context Notes, and Content Manager top-level/revision/course-workspace state parity are done; Content Manager inline feedback parity is active. | Continue deferring Content Manager recovery workflow states unless explicitly scoped.                                                  |
...
Active AW-006 state-primitive implementation candidate after PR `#796/#797`:
...
- Selected by fresh queue/design/code re-audit from clean `main@44df95d`.
