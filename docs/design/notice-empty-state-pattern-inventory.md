...
| Admin note screenshot capture       | `components/admin/AdminNoteScreenshotCaptureButton.tsx`                                                                                                                                                                                  | permission denied, cancelled, unsupported, capture error, preview save error                                                   | Small admin notes utility surface with route-local feedback markup and existing focused unit coverage.                                                                                                                                            | Current bounded slice; align feedback with `AdminManagerState` without changing capture, crop, upload, or recovery behavior. |
...
## Current Admin Note Screenshot Capture Feedback State Parity Slice
...
- Reuse the existing admin-local `AdminManagerState` helper for `AdminNoteScreenshotCaptureButton` recovery and save-error feedback only.
...
- Add focused component/unit coverage around screenshot feedback semantics.
- Use `after/reference` screenshot handoff because rendered admin UI changes.
