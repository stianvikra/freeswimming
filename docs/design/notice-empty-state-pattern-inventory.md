...
| Admin Help/Guide                               | `components/admin/AdminHelpCenter.tsx`, `components/admin/AdminWorkspace.tsx`, `tests/e2e/admin-help-center.spec.ts`                                                                                                                                                                                                                                                                    | operator training shell, mobile quick links, active top desktop rail submenu, dashboard-tab guidance, workflow sections, button glossary, edit-scope guardrails, quality matrix, doc controls, runbook references, services, playbooks, troubleshoot cards, change-governance note                        | Fresh `2026-05-31` re-audit after PR `#916/#917` found the Help/Guide surface still using older local `rounded-2xl`/`slate` cards and anchor styling while the `/admin` shell, admin managers, and contextual notes now use the newer AW-006 admin token/action hierarchy. Screenshot review then identified scroll-to-top friction from keeping all Help/Guide section links only in the top hero.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Active: `docs/task-briefs/in-progress/2026-05-31-aw-006-admin-help-guide-token-action-parity-10-10.md`; preserve Help/Guide content, workflow labels, runbook paths, e2e assertions, admin tab routing, behavior, authz, APIs, and support procedures.                                 |
...
| Admin notes/content managers                   | `components/admin/AdminNotesManager.tsx`, `components/admin/AdminContextNotesPanel.tsx`, `components/admin/AdminContentManager.tsx`                                                                                                                                                                                                                                                     | loading, warning, errors, recovery warnings, upload retry, empty, no matches, contextual note shell/actions, audit mode, focus mode, mirror snapshot, QA cleanup utility states                                                                                                                           | Dense, high-value operator workflows with staged uploads, related records, and recovery behavior. Admin Notes top-level, Context Notes, and Content Manager top-level/revision/course-workspace/inline/course-structure feedback parity are done. PR `#898/#899` closed the bounded `AdminContentManager` utility-state parity slice for audit-mode, focus-mode, platform mirror, and QA cleanup utility surfaces. PR `#914/#915` closed the `AdminContextNotesPanel` token/action parity gap by aligning the contextual notes shell, forms, rows, attachments, and visible actions with the current admin token/action hierarchy while preserving note data, context filters, upload/retry/delete behavior, authz, labels, Help/Guide, support behavior, analytics, and data contracts.                                                                                                                                                                       | Done: `docs/task-briefs/done/2026-05-31-aw-006-admin-context-notes-panel-token-action-parity-10-10.md`; no active admin notes/content manager token/action follow-up is selected until a future queue/design/code re-audit chooses one.                                                |
