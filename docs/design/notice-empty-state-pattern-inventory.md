...
| Admin management panels             | `components/admin/AdminCommerceManager.tsx`, `components/admin/AdminOperationsManager.tsx`, `components/admin/AdminQrLinksManager.tsx`, `components/admin/AdminEmailTemplatesManager.tsx`, `components/admin/AdminCategoriesManager.tsx` | loading, schema warning, load error + retry, action error, action notice, empty list, no matches, QR asset generation feedback | Top-level repeated manager states have mostly moved to the admin-local helper; QR Registry still has a small route-local QR asset loading/error treatment inside preview.                                                                         | Active bounded candidate: migrate QR Registry asset feedback only; do not reopen the completed broad pilot scope. |
...
- selected after a fresh queue/design/code re-audit from clean `main@37736ae` after `#800/#801`.
- scope: reuse the existing admin-local `AdminManagerState` helper for `AdminQrLinksManager` QR asset generation loading and error+retry feedback only.
