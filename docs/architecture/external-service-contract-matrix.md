...
| `analytics_events`              | `lib/analytics/events.ts`, `lib/analytics/persistence.ts`, `/api/analytics/event`, server-side persisted commerce/funnel events                 | Product/ops owner; typed product and operational event taxonomy       | Repo-canonical: event names and sanitizer. Supabase-canonical: sanitized event rows and safe dimensions. No third-party analytics provider is selected.                                                  | `P1`               |
...
| `analytics_events`              | Supabase service-role key is server-only for API-owned event inserts. Optional user ID attachment uses cookie-gated server helper only; public aggregate events keep `user_id=null`. | Event names are typed. Payload sanitizer redacts sensitive key names and drops complex values before persistence. Retrying client events is safe only as duplicate telemetry, not source-of-truth data.                   | Invalid content type/JSON/name fails `415`/`400`; persistence failure logs diagnostics and fails soft; ingestion and admin insights routes are no-store and force-dynamic.                                             | `analytics_events` rows include event name, timestamp, channel, optional user ID, public aggregate flag, safe dimensions, and sanitized payload. No email/token/secret/password/cookie/authorization/raw URL/IP/User-Agent.     | Disable new event call sites by code/flag, rollback deploy, or revert the additive persistence slice. Adding a vendor requires a new matrix row and privacy review before sending payloads externally.     |
