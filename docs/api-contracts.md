...
- If the linked Habit has an unsupported full definition/setup, the primary Micro Session completion still saves and returns `200`. Its nested `habitCredit` result is `status = "blocked"`, `code = "UNSUPPORTED_HABIT_DEFINITION"`; no new Habit check-in or Habit-success event is created.
...
`plan.habitLink.habitDefinitionSupport` is `supported`, `unsupported`, or `unavailable`. `supported` includes canonical definitions and the exact legacy-cadence compatibility form. Only `unsupported` means the owner-scoped Habit row was found but one or more definition semantics need an explicit mapping; `unavailable` preserves the existing missing/schema/query fallback without claiming a future-value conflict. Neither branch exposes a raw unknown value.
...
All currently supported Habit definitions inherit this boundary through the shared definition/check-in contracts. A future type, mode, category, target operator/unit/shape, timer shape, cadence period/policy/count, weekday, or status that changes display or due/done, review, streak, Perfect Day, Micro credit, or Calendar counting requires an explicit database/type/view-model mapping, negative tests, and Help/support update before release; labels may change through shared display mappings without changing stable IDs or historical date keys.

### Supported Habit Definition Boundary

Persisted definitions enter normal Habit semantics only after one shared pre-normalization classifier validates `habit_type`, `habit_mode`, definition `status`, category, the complete target/operator/unit/time/timer shape, cadence period/policy/count, and unique known schedule days. Canonical definitions are supported. The only legacy-compatible form has all three cadence fields nullish together plus one to seven unique known weekdays; seven days resolves to daily/fixed/1 and one to six days resolve to weekly/fixed/the exact day count without rewriting the raw row. Every other unknown, partial, contradictory, malformed, deprecated, or unmapped definition remains server-canonical but is exposed to private product surfaces only as a generic read-only `Needs review` descriptor. Its title and stable ID remain visible to the owner, history remains preserved, and raw unknown values never enter normal UI, analytics, or routine logs.
...
- Explicit unknown/null enum input, malformed supplied scalar or combination, or any supplied `targetOperator` property on create/update returns `400` with `code = "UNSUPPORTED_HABIT_DEFINITION_VALUE"` before a write or success event. Syntactically invalid fields are rejected before definition/cap reads; semantic PATCH combinations are resolved only after the owner-scoped stored row is validated. Documented create omissions and syntactically valid irrelevant carry-over from the seven previous editor payload shapes remain compatible; the current client sends only target/time/timer fields relevant to an actually changed selected shape.
- Omitted PATCH fields inherit the already validated usable row. Unrelated edits write no semantic defaults; a supported `at_most`/`after` operator is preserved while type/mode stays unchanged, and an unchanged legacy cadence remains raw. An explicit valid cadence edit writes one complete canonical tuple.
- `PATCH`, archive/restore, check-in/clear/timer, or stats reset against an owner-scoped unsupported stored definition returns `409` with `code = "UNSUPPORTED_HABIT_DEFINITION"` before insert, update, delete, upsert, snapshot-success analytics, or another Habit mutation.
