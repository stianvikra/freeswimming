...
- AW-009 started as `planned` in backlog and now moved to active execution.
...
- `2026-03-11 | feat/aw-009-email-template-history-analytics-slice-4 | completed AW-009 slice-4 history + observability slice: added template revisions API (`GET /api/admin/email-templates/[id]/revisions`) and admin history panel visibility in Email Templates UI; added lifecycle analytics events (`email_template_saved`, `email_template_published`, `email_template_reverted`) on create/update transitions; updated security/help coverage for new endpoint and history action copy | next: run lint:briefs + verify:pre-pr, then open PR`
