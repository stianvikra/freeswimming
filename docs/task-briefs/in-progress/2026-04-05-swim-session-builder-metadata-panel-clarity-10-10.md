...
- Do not remove the auto-generated step summary/category line shown under each step title in the session builder cards; that line is still part of the collapsed-card readability contract and is out of scope for this metadata-panel slice.
...
- `2026-04-10 | in-progress | confirmed that the manual builder uses manual source fingerprints while the AI generator uses the same editor with copyVariant=generator; this slice will simplify metadata only for manual authoring, keep canonical session metadata fields in the underlying draft contract, and avoid schema changes | next: hide manual-only noise where defaults are already canonical, calm the metadata heading/copy, update tests, and verify save/export compatibility`
- `2026-04-11 | in-progress | locked the decision to keep the auto-generated step summary/category line under each step title because it remains part of the collapsed-card scanability contract; this slice continues to target only the top metadata panel and must not remove that step-card text | next: finish validation, then ship the metadata-panel cleanup without touching step-card summary content`
- `2026-04-11 | in-progress | verify uncovered legacy stored-workout payloads with missing metadata fields (`description`, `sizeMode`, `allowedStrokes`) and even invalid program-library workout rows with no steps; this slice now includes compatibility hardening so manual metadata simplification does not break canonical load paths, and program-library available-workout summaries degrade by skipping invalid legacy rows instead of crashing My Library | next: rerun full verify:pre-pr on the hardened compatibility path, then commit/push if the gate is green`
- `2026-04-11 | in-progress | full verify also exposed two interaction/data-contract regressions around program export: My Library create buttons could be clicked before hydration attached handlers, and the program library was querying workouts with an under-selected shape that stripped out data needed to build valid available-workout summaries; this slice now disables create buttons until client-ready and uses the full workout select for program-library workout loading so export/program assignment flows stay truthful and testable | next: rerun full verify:pre-pr after confirming the flaky unrelated training-context timeout is external/auth-network noise, then commit/push if the gate is green`
- `2026-04-11 | blocked-on-validation | the swim-builder metadata slice now passes targeted unit coverage, targeted builder E2E, and both specific admin-contextual-notes desktop failures when rerun alone on a clean server, but repeated full \`verify:pre-pr\` attempts still stop in unrelated \`admin-contextual-notes\` desktop Chromium flows (`course` and `plans`) that are outside this brief's write scope; treat that suite as an external flake/blocker for this slice rather than a builder regression | next: either harden the flaky admin-contextual-notes suite in a separate follow-up or accept that this branch is locally ready but cannot claim a green full pre-PR gate yet`
- `2026-04-11 | in-progress | hardened the external admin-contextual-notes gate by waiting for the requested course lesson context to resolve before mutating lesson-scoped notes and by retrying quick-capture dialog opening on the plans page; targeted desktop repeats held stable and the full \`npm run verify:pre-pr\` gate passed green afterward, so this slice is no longer blocked on validation | next: commit the gate hardening, push the rebased branch back onto PR #410, and run \`npm run verify:pre-merge\` before merge recommendation`
