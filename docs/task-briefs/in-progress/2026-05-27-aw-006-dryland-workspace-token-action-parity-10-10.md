...
| DevOps and rollback readiness                 | `target`     | Normal git revert restores prior markup/tests/docs and the CI smoke install retry/timeout budget; no migration, dependency, provider setting, or feature flag rollback is needed.       | git diff + validation evidence                 | `5/5`                   |
...
- Required-check CI smoke install retry/timeout budget only if GitHub smoke jobs confirm Playwright Chromium install can hang before tests start.
...
- CI workflow behavior beyond the smoke-job Playwright install retry/timeout budget needed to keep existing required checks runnable.
...
- If CI smoke jobs fail before tests start because Playwright Chromium install hangs or exhausts the job budget, adjust only the affected smoke install timeout/retry budget and rerun required checks.
