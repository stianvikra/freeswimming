...
| DevOps and rollback readiness                 | `target`     | Normal git revert restores prior markup/tests/docs and the CI smoke Chromium install timeout/retry strategy; no migration, dependency, provider setting, or feature flag rollback is needed. | git diff + validation evidence                 | `5/5`                   |
...
- Required-check CI smoke Chromium install timeout/retry strategy only if GitHub smoke jobs confirm Playwright install can hang before tests start.
...
- CI workflow behavior beyond the smoke-job Playwright Chromium install timeout/retry strategy needed to keep existing required checks runnable.
...
- If CI smoke jobs fail before tests start because Playwright Chromium install hangs or exhausts the job budget, adjust only the affected smoke install strategy and rerun required checks.
