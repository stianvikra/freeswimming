...
- `2026-08-31 | authenticated Admin exact-locator follow-up` - corrective commit `2919161b` passed full local `verify:pre-pr` and was pushed with current owner-approved screenshot evidence. Authenticated Admin run `33410536342` reached the deep lesson editor (`8 passed / 1 failed / 1 skipped`) and exposed two strict-mode locator collisions rather than product failures: the canonical runtime ID appeared both as exact code and inside a longer context heading, while accessible-name substring matching made `Next step` also match `Legacy next step fallback`. Both assertions now require exact text/name matches while retaining the same runtime-ID and editable-field contracts; no product/rendering file changed after the approved capture | next: run targeted lint/type validation, full `verify:pre-pr`, commit/push, then require fresh PR CI/Admin E2E/pre-merge evidence.
- `2026-08-31 | exact-locator pre-PR green` - the two assertion-only exact-locator corrections passed ESLint, strict typecheck, brief lint, and the full `verify:pre-pr` lane in `artifacts/test-runs/20260831-175509/verify.log`: `261/261` unit files with `1739/1739` tests, Next `16.3.3` build with `72/72` generated pages, route performance budgets (worst margin `16.8%`, weekly hold `1/2`), and Playwright `111 passed / 567 expected skips`. The only post-gate change is this docs-only evidence checkpoint; no product/rendering file changed after the approved capture | next: commit/push, then require fresh PR CI, authenticated Admin E2E, and `verify:pre-merge`.
- `2026-08-31 | authenticated Admin saved-value locator follow-up` - commit `610e2e28` passed full local `verify:pre-pr`, and fresh ordinary checks were green except the still-running verify job when authenticated Admin run `33411983156` reached the post-save lesson assertions (`8 passed / 1 failed / 1 skipped`). The input locator was exact, but its parallel persisted-value assertion still used accessible-name substring matching and collided with `Legacy next step fallback` in all three attempts. The saved-value assertion now uses the same exact `Next step` name; a complete search confirms these are the only two role/textbox `Next step` locators in the test. No product/rendering file changed after the approved capture | next: run targeted validation and full `verify:pre-pr`, commit/push, then require fresh PR CI/Admin E2E/pre-merge evidence.
