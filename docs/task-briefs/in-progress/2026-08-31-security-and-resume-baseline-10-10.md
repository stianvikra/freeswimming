...
| Testing and QA automation                     | `target`     | Targeted unit/Admin E2E, brief lint, full `verify:pre-pr`, required CI, and `verify:pre-merge` pass with no new skip or assertion weakening.                                      | command logs + GitHub checks                             | `5/5`                   |
...
`N/A` because this maintenance slice changes no server-canonical entity, local browser state, sync trigger, conflict policy, retention rule, database schema, RLS policy, cache ownership, or invalidation behavior.
