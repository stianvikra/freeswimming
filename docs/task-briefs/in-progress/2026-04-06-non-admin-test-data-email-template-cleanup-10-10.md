...
- `2026-04-06 | schema follow-up implementation | added a migration to drop the live revision FK, taught the cleanup route to purge orphaned and post-delete QA revision rows as well as templates, and revalidated via typecheck, targeted vitest, targeted playwright, lint:briefs:all, and full verify:pre-pr (95 passed / 319 skipped); `supabase db push --dry-run` is currently blocked in this session because the repo is not linked to a Supabase project and no DB password/URL is present in repo-local env | next: commit, open PR, merge the follow-up fix, then apply the migration in the linked Supabase environment before rerunning live cleanup and closing the admin note`
