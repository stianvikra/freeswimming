...
- `2026-04-03` — child slice started on branch `feat/workout-builder-notice-placement-2026-04-03` through `docs/task-briefs/in-progress/2026-04-03-workout-builder-notice-placement-and-audience-10-10.md`; this pass narrows the remaining builder notice follow-up to calmer export/handoff placement, truthful optional-support copy, and explicit persistence language without removing any manual swim-session builder input fields. Next step: implement the calm disclosure, refresh unit/e2e coverage, and run `npm run verify:pre-pr`.
...
- `2026-04-02` — child slice started on branch `feat/workout-builder-pool-presets-copy-2026-04-02` through `docs/task-briefs/in-progress/2026-04-02-workout-builder-drill-kick-taxonomy-followup-10-10.md`; the current pass clarifies step-taxonomy labels, applies deterministic kick/drill focus defaults only when the focus field was previously unset, and keeps implied kick/drill tags from duplicating inside summaries. Targeted `vitest` and `typecheck` are green. Next step: run `npm run lint:briefs` and `npm run verify:pre-pr`, then commit/push/open PR if the full gate stays green.
