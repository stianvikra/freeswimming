...
| H-009 | Satisfied for Child A/B   | Child A and Child B each refreshed Habits code, benchmark expectations, and data sufficiency before implementation; future children must repeat their own audit.                                                                                                     |
| H-010 | Partially resolved        | PR `#977` covers rest/skip, consistency %, and current streak where data proves it; PR `#979` covers timer/manual/one-active-timer gaps; backfill, best streak, habit score, calendar/history, notes, archive history, sound, reminders, and export remain deferred. |
...
| H-015 | In progress               | Child D owns `Build` -> `Do` user-facing copy while preserving stored/API `build`.                                                                                                                                                                                   |
| H-016 | In progress               | Child D owns streak readability copy such as `Streak: 20 days`.                                                                                                                                                                                                      |
| H-017 | In progress               | Child D owns count/today progress readability and removal of duplicate competing progress phrasing.                                                                                                                                                                  |
| H-018 | Partially in progress     | Child D owns mobile access to the existing weekly/calendar overview; Child C still owns selecting prior days and backfill/edit-history behavior.                                                                                                                     |
| H-019 | Deferred                  | Child D will present simple water reminders as Done-only in screenshot evidence; explicit `Any amount`, litres, and glasses builder semantics require a future builder target semantics child.                                                                       |
