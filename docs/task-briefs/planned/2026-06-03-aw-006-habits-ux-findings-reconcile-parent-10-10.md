...
| Child F: Habits Builder Target Semantics               | Active: `docs/task-briefs/in-progress/2026-06-04-aw-006-habits-builder-target-semantics-10-10.md`. Decide and implement target-entry options such as Done-only reminders, `Any amount`, safe existing count units, validation copy, count/timed input ergonomics, Habits week-bar click/swipe navigation, right-aligned header pills, Details action hierarchy, and support guidance for water-style habits. `litres` is documented as future migration scope because the current database constraint does not allow it. | History/backfill data model changes, timer behavior, midnight auto-complete/background check-in creation, unsupported unit writes, broad card redesign, or cross-product calendar behavior.                   |
...
| H-019 | In progress                 | Child F is active at `docs/task-briefs/in-progress/2026-06-04-aw-006-habits-builder-target-semantics-10-10.md`; safe scope is Done-only/Any amount and existing count units while `litres` remains future migration scope.                                                                                                                                  |
...
| H-021 | In progress                 | Child F owns count row input, `Save`, and `Details` alignment.                                                                                                                                                                                                                                                                                              |
| H-022 | In progress                 | Child F owns numeric stepper ergonomics for target/check-in rows without changing persisted semantics.                                                                                                                                                                                                                                                      |
| H-023 | In progress                 | Child F owns the timed progress visual grouping while preserving Child B timer behavior.                                                                                                                                                                                                                                                                    |
| H-024 | In progress                 | Child F owns peer action min-width/equal mobile behavior for changed Habits rows.                                                                                                                                                                                                                                                                           |
| H-025 | In progress                 | Child F owns Habits week-bar day click, previous/next click fallback, and intended swipe navigation on the blue week overview container only.                                                                                                                                                                                                               |
| H-026 | In progress                 | Child F owns habit row header pill alignment back to a right-aligned title/pills scan hierarchy.                                                                                                                                                                                                                                                            |
| H-027 | In progress                 | Child F owns Details action hierarchy: primary completion first, timed `Finish` blue, `Rest day` secondary, and `Reset` lower priority.                                                                                                                                                                                                                     |
...
- `2026-06-04 | in-progress | owner explicitly selected Child F Builder Target Semantics and added screenshot findings for count row Save/Details alignment, larger numeric steppers, timed progress hierarchy, and equal-feeling paired buttons; active child is docs/task-briefs/in-progress/2026-06-04-aw-006-habits-builder-target-semantics-10-10.md on branch aw-006-habits-builder-target-semantics | next: implement focused target/row ergonomics, run targeted validation, capture screenshot handoff, and stop for owner approval before npm run verify:pre-pr`
