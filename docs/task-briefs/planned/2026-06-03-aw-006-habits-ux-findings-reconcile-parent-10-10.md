...
| H-036 | Timed manual time should be an absolute editable per-habit/per-date source, not an additive "add again" command.                                                         | Changing manual time from `2` to `5` can otherwise double-count as `7`, making timed history untrustworthy.               | Active child: Timed Manual Time + Pending Date + Analysis Shortcut.                            |
| H-037 | Habits date clicks should distinguish requested/pending date from loaded/confirmed date and fail visibly on load errors.                                                 | Stale habit data can appear fully selected for a date that has not loaded or failed to load.                              | Active child: Timed Manual Time + Pending Date + Analysis Shortcut.                            |
| H-038 | Mobile active Habits needs a graph/analysis shortcut beside the calendar control using canonical calendar params.                                                        | Users can miss Habits comparison analysis or navigate through extra surfaces; hardcoded links can drift from calendar IA. | Active child: Timed Manual Time + Pending Date + Analysis Shortcut.                            |
...
It closed the first-pass timer/manual UX, single running timer behavior, and duplicate timer display cleanup while keeping sound out of the first pass; active Child I supersedes the old additive manual edit model with explicit source fields.
...
Child I status: in progress on branch `aw-006-habits-timed-manual-pending-analysis`, with screenshot handoff required before broad PR gates.

## Child Return Status

| ID    | Status after latest child   | Evidence / Next owner decision                                                                                                                                                                                                                                                                                                                                |
| ----- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| H-001 | Superseded by H-036 active  | PR `#979` first made today's timed total include saved server minutes plus current local timer; active Child I now replaces additive manual semantics with explicit timer/manual source fields and absolute manual edits.                                                                                                                                     |
| H-002 | Superseded by H-036 active  | PR `#979` replaced unclear manual labels with `Manual time` / `Add manual time`; active Child I keeps `Manual time` but changes the command to an absolute `Save manual time` workflow.                                                                                                                                                                       |
...
| H-036 | In progress                 | Active Child I adds `timer_seconds` and `manual_minutes`, validates whole-minute manual edits including `0`, preserves legacy numeric rows, and shows separate timer/manual sources with one total.                                                                                                                                                           |
| H-037 | In progress                 | Active Child I tracks requested vs confirmed selected date, amber pending state, failed-date fallback, and tests stale-data failure behavior.                                                                                                                                                                                                                 |
| H-038 | In progress                 | Active Child I adds the mobile graph shortcut with accessible name `View Habits analysis` and canonical `/my-library/calendar?source=habits&period=week&date=<selectedDate>` params.                                                                                                                                                                          |
...
- `2026-06-05 | in-progress | owner approved active Child I Timed Manual Time + Pending Date + Analysis Shortcut after PR #991/#992 closeout; added H-036/H-037/H-038 and superseded the old additive manual-time return status while preserving Child B history | next: finish implementation, targeted validation, screenshot handoff, and stop for owner visual approval before npm run verify:pre-pr`
