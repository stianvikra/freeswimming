...
- `reason`: The original review lived only in chat after `main@be554e9`; shipped follow-ups through `#832` are recorded below. Repo-managed closeout for `#833` leaves no active AW-006 implementation slice until the next fresh queue/design/code re-audit.
- `must_refresh_before_execution_if`: Refresh if AGENTS.md, scorecard categories, AW-006 scope, mobile nav, auth sign-in, course/player, plans/payment copy, `/about`, design tokens, admin state primitives, dryland routes/micro-session state contracts, `/my-library/goals`, goals API/storage contracts, task-brief closeout rules, forward compatibility rules, screenshot handoff rules, or verification lanes change before the next UX/UI slice starts.
...
PR `#834` shipped the selected My Training Feedback Semantics slice. This repo-managed closeout moves the brief to `done`, updates stale queue/inventory references, and leaves no active AW-006 implementation slice selected.
