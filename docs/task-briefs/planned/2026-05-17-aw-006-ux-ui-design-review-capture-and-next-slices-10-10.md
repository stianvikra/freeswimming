...
| -------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------- |
| Next AW-006 UI slice | `unselected` | Run a fresh canonical queue/design inventory re-audit after the Admin Notes closeout before selecting the next bounded PR-sized slice. | this queue brief, `docs/design/notice-empty-state-pattern-inventory.md`, next active brief after creation | Do not start another UI slice until the closeout PR is merged, `npm run post-merge:preflight` is clean, and the next scope has a non-programmer explanation. | Required for any future UI slice. |
...
- Continue bounded admin-local primitive parity only where a mature reference surface already exists. Status: `done` for contextual QR panel, Admin Notes Manager, and closeout queue/gate repair; next candidate must come from a fresh re-audit.
