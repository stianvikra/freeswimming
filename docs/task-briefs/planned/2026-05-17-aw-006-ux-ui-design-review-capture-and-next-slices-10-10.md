...
- `reason`: The original review lived only in chat after `main@be554e9`; shipped follow-ups through `#828` are recorded below. Repo-managed closeout for `#828` leaves no active AW-006 implementation slice until the next fresh queue/design/code re-audit.
- `must_refresh_before_execution_if`: Refresh if AGENTS.md, scorecard categories, AW-006 scope, mobile nav, auth sign-in, course/player, plans/payment copy, `/about`, design tokens, admin state primitives, dryland routes/micro-session state contracts, task-brief closeout rules, forward compatibility rules, screenshot handoff rules, or verification lanes change before the next UX/UI slice starts.
...
PR `#830` shipped the selected My Swim Profile section feedback semantics slice. Repo-managed closeout leaves no active AW-006 implementation slice until the next fresh queue/design/code re-audit.
