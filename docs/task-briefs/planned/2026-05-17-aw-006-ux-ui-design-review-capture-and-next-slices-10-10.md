...
| Auth feedback source-of-truth cleanup       | `current` | `docs/task-briefs/in-progress/2026-05-19-aw-006-auth-feedback-source-of-truth-cleanup-10-10.md`                      | Cleans up overlapping auth feedback ownership so `/auth/sign-in` keeps one maintained request-feedback source and stable cooldown hydration without changing auth behavior, cooldown rules, redirects, or copy.                                  |
...
| ------------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| Auth feedback source-of-truth cleanup | `current` | Remove overlapping unused auth feedback code, stabilize cooldown hydration, and add focused tests so `/auth/sign-in` has one maintained request-feedback source. | `app/auth/sign-in/page.tsx`, `components/auth/`, `lib/auth/sign-in-ui-state.ts`, `tests/unit/`, this queue brief, `docs/design/notice-empty-state-pattern-inventory.md` | Preserve auth server actions, OTP/magic-link rules, redirects, provider diagnostics, cooldown cadence, labels, Help/Guide, Stripe, Supabase, analytics, and private-gate behavior. | Required before `verify:pre-pr`. |
