...
- `reason`: The original review lived only in chat after `main@be554e9`; shipped follow-ups through `#819` are recorded below. The queue now points at a bounded post-Dryland lifecycle/tooling repair before any next AW-006 product implementation slice is selected.
...
After PR `#820`, the post-Dryland lifecycle/tooling repair is complete. No next AW-006 product implementation slice is selected until a fresh queue/design/code re-audit is completed.
