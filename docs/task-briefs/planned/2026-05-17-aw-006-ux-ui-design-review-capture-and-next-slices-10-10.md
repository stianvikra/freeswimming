...
| ------------------------------------------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| Admin Notes Manager top-level state parity | `current` | Bring Admin Notes top-level warning, loading, load error+retry, action feedback, empty, and no-results states into admin primitive parity. | `docs/task-briefs/in-progress/2026-05-20-aw-006-admin-notes-manager-top-level-state-primitive-parity-10-10.md`, `components/admin/AdminNotesManager.tsx`, `components/admin/AdminManagerState.tsx`, `tests/unit/admin-notes-manager*.tsx`, this queue brief, design inventory | Preserve notes API payloads, attachment upload/recovery, related-note linking, filters/search params, note sorting, copy, retry callbacks, authz, Help/Guide, and support paths. | Required before `verify:pre-pr`. |
...
- Continue bounded admin-local primitive parity only where a mature reference surface already exists. Status: `done` for contextual QR panel and closeout queue/gate repair; current candidate is Admin Notes Manager top-level states.
