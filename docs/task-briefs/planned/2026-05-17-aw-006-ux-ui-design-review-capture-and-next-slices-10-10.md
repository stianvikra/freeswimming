...
Admin Content Manager top-level state parity shipped through `#790/#791`. No next AW-006 UI implementation slice is selected. The closeout reference guard must make stale queue and design-inventory status references fail automatically before the next fresh UI-slice re-audit.

| Slice                           | Status   | Objective                                                                                                 | Likely files                                                                                                                                                                                                     | Risks / protected areas                                                                                                                                   | Screenshot gate                  |
| ------------------------------- | -------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| AW-006 closeout reference guard | `active` | Detect stale completed-slice active/current/candidate references in canonical queue and design inventory. | `scripts/lint-task-brief-scorecard.mjs`, `scripts/post-merge-preflight.mjs`, `tests/unit/task-brief-scorecard-lint.test.ts`, `tests/unit/merge-preflight.test.ts`, this queue brief, active implementation brief | Preserve runtime UI, product behavior, APIs, Supabase, auth, Stripe, analytics, Help/Guide, and workflow labels; do not select a new UI slice in this PR. | Not required: docs/tooling only. |
...
- Continue bounded admin-local primitive parity only where a mature reference surface already exists. Status: `done` for contextual QR panel, Admin Notes Manager, contextual notes panel, and Admin Content Manager top-level state parity; next UI slice is intentionally unselected until the closeout reference guard is complete.
