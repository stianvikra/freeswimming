...
Close out Admin Messages v1 with launch-grade operations, test coverage, env parity, runbooks, support diagnostics, and rollback evidence before test swimmers use the intake flow.

## Dependency Order

- Parent: `docs/task-briefs/planned/2026-05-06-admin-message-management-parent-10-10.md`
- Must follow:
  - `docs/task-briefs/in-progress/2026-05-06-admin-message-delivery-provider-contract-10-10.md`
...
- All message-management child slices have evidence.
...
- Support runbook answers:
  ...
  - did admin read it,
  - did we reply,
  - did provider accept the outbound email,
  - what failed and how to retry/rollback.
...
| --------------------------------------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ----------------------- |
| Product goals and IA                          | `target`     | Admin Messages v1 has a clear closeout checklist and no open blocker before test swimmer intake.                            | closeout audit                        | `5/5`                   |
...
| Visual design quality                         | `target`     | Final inbox/reply screenshots show polished admin UI across representative desktop/mobile widths.                           | screenshot handoff                    | `5/5`                   |
| Business logic correctness and data integrity | `target`     | Intake, status, archive/delete, reply, retry, and delivery attempts are covered by deterministic tests and closeout review. | test evidence + data audit            | `5/5`                   |
| Admin editor ergonomics                       | `target`     | Admin can complete high-frequency triage and reply flows without hidden prerequisites or ambiguous states.                  | manual admin QA                       | `5/5`                   |
...
| Reliability and failure handling              | `target`     | DB, provider, authz, validation, rate-limit, retry, and rollback failure modes are tested and documented.                   | negative-path tests + runbook         | `5/5`                   |
...
| Content governance                            | `target`     | Message/reply lifecycle, source variants, statuses, and provider attempt evidence have one governed source of truth.        | lifecycle audit                       | `5/5`                   |
...
| Analytics and KPI observability               | `target`     | Safe event taxonomy and admin/ops counts exist for intake, triage, reply, failure, retry, archive/delete.                   | event tests/dashboardability review   | `5/5`                   |
...
- Server-canonical:
  - final closeout confirms messages, replies, statuses, and delivery attempts are canonical in app storage.
- Provider-canonical:
  - provider IDs/statuses are secondary references only.
- Local-only:
  - public/admin UI drafts and filters only.
- Sync policy:
...
- Canonical stable ID:
  - message, reply, and delivery attempt IDs.
...
- Observability and repair:
  - runbook defines repair for failed attempts, missing provider config, and stuck statuses.
...
3. Admin can diagnose intake/reply/provider status from dashboard and runbooks.
