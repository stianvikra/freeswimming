...
| DevOps and rollback readiness                 | `target`     | Normal git revert restores prior markup/tests/docs and the CI smoke Chromium install/sentinel strategy; no migration, dependency, provider setting, or feature flag rollback is needed. | git diff + validation evidence                 | `5/5`                   |
...
- Required-check CI smoke Chromium install/sentinel strategy only if GitHub smoke jobs confirm Playwright install can hang before tests start.
...
- CI workflow behavior beyond the smoke-job Playwright Chromium install/sentinel strategy needed to keep existing required checks runnable.
...
- If CI smoke jobs fail before tests start because Playwright Chromium install hangs or exhausts the job budget, adjust only the affected smoke install/sentinel strategy and rerun required checks.
