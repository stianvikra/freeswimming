...
- `2026-04-25 | in-progress | follow-on hardening landed for transient My Library verification surfaces: dev-login now retries transient Supabase/auth fetch failures, program export preview uses explicit timeout + retry instead of hanging on canonical export loading, navigation settle helpers were hardened for repeated route retries, and CreateManualWorkoutButton now clears delayed fallback navigation on unmount; local lint/typecheck/unit/build/perf are green, the previously failing My Library desktop Playwright reproductions passed targeted reruns, but fresh local Playwright runs are currently hanging silently after startup on this machine and need CI confirmation on PR #512 | next: commit these hardening changes, push the branch, refresh PR evidence, and use GitHub verify/required checks plus local verify:pre-merge once CI confirms the suite on a clean runner`
