...
- CI/deploy workflow evidence: PR #1167 CI exposed a deterministic `deploy-preview` failure from Vercel CLI upload limits (`api-upload-free`) after the preview build completed and the separate Vercel check passed. The workflow fix is limited to `.github/workflows/vercel-preview.yml`, adding `--archive=tgz` to `vercel deploy --prebuilt` as recommended by the failing log; no product runtime files, visual files, screenshots, or Vercel project settings changed after screenshot approval.
