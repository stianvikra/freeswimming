...
- `2026-03-25 | merged | workout-level Garmin-ready JSON export merged earlier as \`2c4fcd8\`, workout PDF print export merged as \`e938560\`, required CI checks passed, and local \`npm run verify:pre-merge\` passed on the green PR head; lifecycle moved from \`in-progress\` to \`done\` and remaining program-level export scope was split into \`docs/task-briefs/blocked/2026-03-25-program-export-adapters-garmin-ready-pdf-followup-10-10.md\` after reconfirming that canonical program generation/storage is still upstream-blocked | next: resume this export track only after canonical program entities/surfaces exist`
