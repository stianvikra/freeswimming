...
- `N/A` for this slice's first workout-PDF delivery because it adds a self-describing print/export affordance inside the dedicated workout builder only, without changing admin/operator workflows or introducing a separate Help/Guide surface outside the in-product copy.
...
- `2026-03-25 | merged | workout-level Garmin-ready JSON export merged earlier as \`2c4fcd8\`, workout PDF print export merged as \`e938560\`, required CI checks passed, and local \`npm run verify:pre-merge\` passed on the green PR head; lifecycle moved from \`in-progress\` to \`done\` and remaining program-level export scope was split into \`docs/task-briefs/in-progress/2026-03-25-program-export-adapters-garmin-ready-pdf-followup-10-10.md\` after the canonical program foundation later shipped and unblocked this downstream track | next: keep workout-level export stable while the reopened program-level follow-up lands on top of the shared foundation`
