...
- `merged_pr`: `#1145`
- `squash_commit`: `fbf0eea8`
- `result`: Closed the course lesson canonical route and multilingual AI-discoverable SEO slice. Public course overview and lessons now have canonical `/en/course` path URLs, metadata, JSON-LD, sitemap coverage, public crawler policy, and legacy query compatibility without duplicate indexable lesson pages.
- `validation`: `npm run verify:pre-pr` passed locally on the full lane for `124e64e7`; PR CI passed `verify`, `size-check`, `e2e-smoke`, `site-lock-smoke`, CodeQL, deploy-preview, and Vercel; `npm run verify:pre-merge` passed before merge readiness.
- `10/10 claim`: yes - all critical target categories reached `5/5`; external Search Console, Bing, and Rich Results evidence remains optional post-deploy evidence when owner credentials are available.

| Category                                      | Achieved Score | Evidence                                                                                                                            | Gaps / Notes                                                                              |
| --------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Product goals and IA                          | `5/5`          | Canonical `/en/course` overview and lesson hierarchy implemented and validated by route/unit/e2e coverage plus PR CI.               | No runtime gap.                                                                           |
| UX flow clarity                               | `5/5`          | Legacy `/course?lesson=` redirects, canonical internal links, share links, QR defaults, and course route expectations validated.    | No runtime gap.                                                                           |
| Business logic correctness and data integrity | `5/5`          | Stable runtime ID to slug/URL helpers, unknown route handling, alias compatibility, and metadata tests passed.                      | No runtime gap.                                                                           |
| Performance (CWV + payloads)                  | `5/5`          | Perf budgets passed locally with `/course` JS about 320.1kb, LCP about 140ms, CLS 0.000, TBT 0.0ms.                                 | Perf trend recommends tightening after 10 green weeks; held for a separate bounded slice. |
| Data placement and sync boundaries            | `5/5`          | Canonical URLs and SEO artifacts derive from typed course helpers and public course data, not user/admin/private state.             | No runtime gap.                                                                           |
| Caching and invalidation strategy             | `5/5`          | App Router metadata, sitemap, robots, static route generation, and private-mode behavior covered by build and tests.                | No runtime gap.                                                                           |
| Reliability and failure handling              | `5/5`          | Unknown slugs/locales and legacy query fallback behavior are deterministic and tested.                                              | No runtime gap.                                                                           |
| Security and authz                            | `5/5`          | Private/admin/member URLs stay out of sitemap/schema/metadata; site-lock/private-mode checks passed in CI.                          | No runtime gap.                                                                           |
| Privacy and compliance                        | `5/5`          | Metadata/schema/crawler surfaces contain public lesson data only; no private user/admin note data added.                            | No runtime gap.                                                                           |
| Content governance                            | `5/5`          | Slug, locale, metadata, crawler, support, and rollback rules documented in the brief and runbook update.                            | No runtime gap.                                                                           |
| SEO and crawlability                          | `5/5`          | Canonicals, sitemap, robots, metadata, JSON-LD, internal links, redirects, and indexability aligned and validated.                  | External Search Console/Bing/Rich Results checks not run locally.                         |
| AI discoverability                            | `5/5`          | `OAI-SearchBot` public policy, deliberate `GPTBot` disallow policy, structured data, and visible lesson text alignment validated.   | External AI/search crawler evidence remains post-deploy optional evidence.                |
| Incident response and support operations      | `5/5`          | Support and rollback notes added for canonical URL, redirect, private-mode, and sitemap diagnostics.                                | No runtime gap.                                                                           |
| i18n operational readiness                    | `5/5`          | English is active/indexable; `nb` readiness is typed but non-indexable until translated content exists; `hreflang` behavior tested. | Future `nb` publish still requires explicit owner/content readiness.                      |
| Stack-fit and dependency discipline           | `5/5`          | Uses existing Next.js App Router, TypeScript helper, course runtime identity, and no new dependency.                                | No runtime gap.                                                                           |
| Testing and QA automation                     | `5/5`          | Local full lane, targeted route/metadata/unit/e2e, PR CI, and pre-merge gate passed.                                                | No runtime gap.                                                                           |
| Scalability and cost efficiency               | `5/5`          | New English lessons derive URLs, metadata, sitemap entries, and schema fields from data/helpers without external runtime services.  | New locale/crawler policy still requires explicit mapping.                                |
| DevOps and rollback readiness                 | `5/5`          | PR body, brief, runbook notes, CI, and pre-merge gate document rollout and rollback path.                                           | No runtime gap.                                                                           |
