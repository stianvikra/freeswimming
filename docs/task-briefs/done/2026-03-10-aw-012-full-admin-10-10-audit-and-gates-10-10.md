...
- `2026-03-13 | chore/github-actions-node24-runtime-2026-03-13 | completed low-risk CI/runtime hygiene follow-up: upgraded GitHub Actions workflow dependencies (`actions/checkout`, `actions/setup-node`, `actions/upload-artifact`) to current Node 24-compatible majors while keeping app/runtime `node-version: 20` unchanged, then hardened the slowest matrix paths (`tests/e2e/admin-content-api-guards.spec.ts`, `tests/e2e/course-nav-contextual.spec.ts`, `tests/e2e/course-support-card-actions.spec.ts`, `tests/e2e/install-prompt.spec.ts`) with explicit slow-budget handling so local `verify:pre-pr` remained deterministic under the full browser matrix | next: rerun verify:pre-pr on final diff, open PR in Safari, run gate:pre-merge, then merge if required CI stays green`
- `2026-03-13 | chore/github-actions-node24-runtime-2026-03-13 | started low-risk CI/runtime hygiene follow-up after merged admin/env-parity slices exposed GitHub Actions Node 20 deprecation warnings; upgrading workflow actions (`checkout`, `setup-node`, `upload-artifact`) to current Node 24-compatible majors while intentionally keeping app/runtime `node-version: 20` unchanged | next: run verify:pre-pr, open PR in Safari, run gate:pre-merge, then merge if required CI stays green`
- `2026-03-13 | feat/admin-foundation-gate-hardening-2026-03-13 | completed post-closeout AW-012 gate-hardening follow-up: split the long shared-editor admin foundation workflow into one role-safe browse/smoke path plus one admin-only mutable lifecycle path, so local `verify:pre-pr`no longer depends on editor sessions attempting cleanup they cannot perform; targeted validation: admin-foundation (3 passed, 1 skipped), drawer-focus-trap desktop-webkit (1 passed), full`npm run verify:pre-pr`PASS (81 passed, 207 skipped) | next: run`npm run gate:pre-merge`, open PR in Safari, merge hardening slice, then rerun local pre-merge gate on env-parity PR #209`
