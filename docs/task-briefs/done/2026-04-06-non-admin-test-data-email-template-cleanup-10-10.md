...
Production email-template data stays trustworthy because Playwright/admin QA no longer leaves test-created `admin_email_templates` or matching revision-history rows behind, and the existing polluted legacy rows have an explicit safe cleanup path.
...
- Follow-up blocker discovered during live cleanup on `2026-04-06`:
  - `admin_email_template_revisions` was created with a live foreign key back to `admin_email_templates`,
  - delete-trigger revision logging then fails on QA cleanup because a historical delete revision cannot safely reference a row that has just been deleted,
  - unlike `admin_content_revisions`, the email-template revision table therefore needs a delete-safe schema contract before production cleanup can finish.
...
- This slice owns:
  ...
  - delete-safe email-template revision history for cleanup-triggered deletes,
  - cleanup of matching QA/test revision rows as well as live template rows,
...
| Business logic correctness and data integrity | `target`     | Cleanup targets only explicit QA/test template keys (new canonical prefix + legacy `aw012_publish_fallback_*`), revision logging remains delete-safe, and normal operator template keys are never matched. | unit tests + production query + code review   |
...
- Retention and safety:
  ...
  - email-template revision rows are historical records and therefore cannot depend on a live foreign key to the template row,
  - QA/test cleanup must explicitly remove matching revision rows in addition to live QA/test templates.
...
- Make email-template revision logging delete-safe for cleanup-triggered deletes.
...
3. Email-template revision logging remains delete-safe during cleanup, and matching QA/test revision rows are also removed.
4. `tests/e2e/admin-email-templates-preview.spec.ts` no longer leaves durable shared template rows behind across repeated runs.
5. Production cleanup can remove the current legacy artifact rows without touching normal operator templates.
...
- `2026-04-06 | merged follow-up discovery | merged the prevention slice via PR #368 (`17a1dfd`) and attempted live production cleanup; the delete path surfaced a schema bug where email-template revision history still enforces a live FK to deleted template rows, so cleanup cannot finish safely until the revision contract matches the delete-safe content-revision pattern | next: ship a small follow-up migration + route cleanup pass for revision rows, rerun targeted validation, deploy, purge the live residue, and only then close the admin note`
- `2026-04-06 | schema follow-up implementation | added a migration to drop the live revision FK, taught the cleanup route to purge orphaned and post-delete QA revision rows as well as templates, and revalidated via typecheck, targeted vitest, targeted playwright, lint:briefs:all, and full verify:pre-pr (95 passed / 319 skipped); `supabase db push --dry-run` was initially blocked because the repo was not linked to a Supabase project in this worktree | next: link the correct worktree, apply the migration in prod, rerun cleanup, and close the note once live residue is gone`
- `2026-04-06 | production closeout | linked the active worktree to Supabase prod, ran `supabase db push`, applied the delete-safe revision migration, and used the service-role cleanup path to remove `306` QA/test `admin_email_templates` rows plus `609` matching `admin_email_template_revisions` rows; post-cleanup verification returned `0` template candidates and `0` revision candidates, and PR #369 reached full green required CI | next: merge PR #369 and close admin note `f96e7d7c-3477-417d-b96f-f2c8f876e2ab``
