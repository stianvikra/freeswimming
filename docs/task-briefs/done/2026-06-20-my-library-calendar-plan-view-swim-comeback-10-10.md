...
- `merged_pr`: `#1187`
- `squash_commit`: `7aa4737f`
- `result`: Closed Child A by adding server-canonical program start weeks, materialized planned workout instances, and a read-only Calendar Plan view while preserving Compare and deferring completion, Garmin, month, and daily-layer behavior to planned child briefs.
- `validation`: Targeted Child A unit suite passed (`12` files, `121` tests); `npm run verify:pre-pr` passed full lane; PR CI passed after one non-reproducible unit flake rerun (`verify`, `e2e-smoke`, `site-lock-smoke`, Vercel, CodeQL, size); `npm run verify:pre-merge` passed full public lane with lint, typecheck, unit, build, performance budgets, and Playwright (`111 passed`, `567 skipped` for local dev-login/browser coverage). Private-gate regression was skipped by the gate because `SITE_LOCK_ENABLED!=1`.
- `screenshots`: owner approved `output/calendar-plan-child-a-2026-06-20-052215` before broad PR gates.
- `performance ratchet`: held. Perf budget script recommended tightening after consecutive green runs, but the separate performance-ratchet brief still waits for at least two new green weekly cycles after `2026-06-19`.
- `10/10 claim`: yes for Child A scope. Critical target categories are all `5/5`: Product goals and IA, UX flow clarity, Business logic correctness and data integrity, Data placement and sync boundaries, Reliability and failure handling, Security and authz, and Testing and QA automation.

| Category                                      | Achieved Score | Evidence                                                                                                    | Gaps / Notes                                                                  |
| --------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Product goals and IA                          | `5/5`          | Plan/Compare route mode, My Library copy, page tests, PR #1187.                                             | Month/today overview is deferred to Child B.                                  |
| UX flow clarity                               | `5/5`          | Owner-approved screenshots and Plan week/session/edit affordance tests.                                     | Completion actions intentionally absent until Child D.                        |
| Visual design quality                         | `5/5`          | Screenshot handoff and responsive action-layout corrections approved.                                       | No remaining Child A visual gap.                                              |
| Business logic correctness and data integrity | `5/5`          | Program `startsOn`, planned-instance sync, date math, and missing-reference tests.                          | Per-instance move/edit semantics deferred to Child C.                         |
| Accessibility (a11y)                          | `5/5`          | Semantic links/buttons, mode navigation tests, and full Playwright gate.                                    | Private signed-in a11y paths skip when dev-login is unavailable locally.      |
| Performance (CWV + payloads)                  | `5/5`          | `npm run test:perf:budgets` passed in pre-PR and pre-merge.                                                 | Ratchet tightening held by separate governance rule.                          |
| Data placement and sync boundaries            | `5/5`          | Migration, DB types, server-canonical planned instances, URL-only view/date state.                          | Actual history remains outside this child.                                    |
| Caching and invalidation strategy             | `5/5`          | Dynamic private route and program save/sync reload behavior validated through page/server tests.            | No extra cache layer introduced.                                              |
| Reliability and failure handling              | `5/5`          | Schema-not-ready, missing start week, missing workout, unsupported params, and load-state tests.            | No remaining Child A reliability gap.                                         |
| Security and authz                            | `5/5`          | Existing auth redirect preserved; owner-scoped program/workout reads and API negative-path coverage passed. | No new cross-user exposure path.                                              |
| i18n operational readiness                    | `5/5`          | Copy uses stable labels and avoids identity coupling to titles/slugs.                                       | Future localization remains explicit mapping work.                            |
| Stack-fit and dependency discipline           | `5/5`          | Reused App Router, Supabase helpers, program contracts, My Library UI primitives, and no new dependency.    | No remaining Child A stack gap.                                               |
| Testing and QA automation                     | `5/5`          | Targeted unit tests, full local gates, CI, and pre-merge passed.                                            | One CI unit flake reran green and reproduced green locally.                   |
| DevOps and rollback readiness                 | `5/5`          | Nullable migration, linked Supabase dry-run parity, PR rollback evidence, and post-merge closeout path.     | Rollback would revert PR #1187 and leave no completion/history data attached. |
