...
- `2026-04-14 | in-progress | found and fixed a real same-HEAD reuse bug during closeout: full-lane pre-merge fallback was calling bare \`npm run verify\` instead of the metadata-writing wrapper, which prevented the next same-HEAD reuse decision from ever turning green | next: validate the fix with targeted tests, rerun pre-merge gate once on current HEAD, then confirm fast-path reuse on the following verify`
