...
- for UI/export bugs that survive repeated attempts or contradict screenshot claims:
  - restate the exact symptom,
  - list likely causes in ranked order,
  - verify/eliminate causes one by one with deterministic probes,
  - validate the actual consumed artifact, not only the preview,
  - log high-cost/reusable findings in `docs/runbooks/high-cost-debug-log.md`
