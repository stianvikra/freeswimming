...
| `INCIDENT_ALERTS_ENABLED`                 | server config        | `optional`    | `optional`    | `optional`    | Optional kill-switch for server-side incident alert emails; defaults to enabled unless set to `0`, `false`, `no`, or `off`. |
| `INCIDENT_ALERT_TO_EMAIL`                 | server config        | `optional`    | `recommended` | `recommended` | Dedicated incident alert mailbox; falls back to `CONTACT_TO_EMAIL` when omitted.                                            |
| `INCIDENT_ALERT_DEDUPE_WINDOW_SECONDS`    | server config        | `optional`    | `optional`    | `optional`    | Incident email dedupe TTL; defaults to `900` seconds and is clamped between 60 seconds and 24 hours.                        |
