...
- Habits now support `Build`, `Quit`, and `Timed` modes plus explicit cadence fields: `cadence_period`, `cadence_target_count`, `cadence_day_policy`, and legacy-compatible `schedule_days`. `Build` habits use quick `Done`/`Undo` check-ins; `Quit` habits show days since the selected quit date and only write rows when the user opens details and logs a user-facing `Log slip` action; `Timed` habits use the local timer or manual minutes and save the final duration as a check-in. Habit row cadence chips such as `Daily`, `3x/week any days`, or `2 fixed days/week` describe the target schedule; `7-day minutes` and `7-day count` are report rollups, not weekly goals.
- If habit ordering looks wrong: active rows are intentionally grouped as due `Build`, due `Timed`, `Quit status`, then later/not-due rows. Inside each group, cadence sorts daily, weekly, monthly, with `sort_order` as the stable tie-breaker. Weekly cadence labels use a calendar week; rolling 7-day summary cards stay labelled `7-day`.
