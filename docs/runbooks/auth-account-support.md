...
- If `Habits` shows "still syncing": verify the linked Supabase environment has applied `20260510153000_habits_perfect_day_foundation.sql` and `20260512103000_habits_v2_build_quit_timed_tracking.sql`, then confirm `habit_definitions` and `habit_check_ins` RLS allow only owner-scoped authenticated reads/writes.
- Habits now support `Build`, `Quit`, and `Timed` modes. `Build` habits use quick `Done`/`Undo` check-ins; `Quit` habits show days since the selected quit date and only write rows when the user logs a user-facing `Log slip` action; `Timed` habits use the local timer or manual minutes and save the final duration as a check-in. Habit row cadence chips such as `Daily` describe the habit target schedule; `7-day minutes` and `7-day count` are report rollups, not weekly goals.
