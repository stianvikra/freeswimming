...
- If a user edits a saved Dryland Session that feeds the current Micro Session: explain that `Save session` applies to future Micro Sessions by default. Use `Update current micro session` only after saving when they deliberately want remaining queued units rebuilt; completed/skipped units are preserved.
