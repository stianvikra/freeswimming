...
- Habits now support user-facing `Do`, `Quit`, and `Timed` modes plus explicit cadence fields: `cadence_period`, `cadence_target_count`, `cadence_day_policy`, and legacy-compatible `schedule_days`. `Do` is stored internally as `habit_mode = build` for compatibility. The add form is collapsed by default behind `Add habit`, active rows stay visible first, and the form can be closed without changing saved habits. `Do` habits support no-quantity `Done only` check-ins plus count/time variants; row-level `Edit` is inside `Details` so the active row stays execution-first. `Quit` habits show days since the selected quit date and only write rows when the user opens details and logs a user-facing `Log slip` action; `Timed` habits use the local timer or manual minutes and save the final duration as a check-in. Habit row cadence chips such as `Daily`, `Weekly - any day`, or `Weekly - 2 fixed days` describe the target schedule; `7-day minutes` and `7-day count` are report rollups, not weekly goals.
- If habit ordering looks wrong: active rows are intentionally grouped as due `Do`, due `Timed`, `Quit status`, `Done today`, `Done this week` / `Done this month`, then later/not-due rows. Inside non-urgent groups, cadence sorts daily, weekly, monthly, with `sort_order` as the stable tie-breaker. Weekly cadence labels use a calendar week; rolling 7-day summary cards stay labelled `7-day`.
...
- If a habit check-in is wrong for today: use `Reset` on the habit row, then log the correct count, minutes, time, or done state again. Duplicate same-day build/timed check-ins should update the same `user_id` + `habit_id` + `check_in_date` row, not create multiple competing facts.
- If a quit habit shows the wrong days-since count: verify `habit_definitions.start_date`, `habit_definitions.last_lapse_date`, and any same-day `habit_check_ins.value_boolean = false` lapse row. Use `Undo slip` if today was logged by mistake; do not ask the user to describe the sensitive habit label.
- If a timed habit timer fails or the browser closes before saving: the live timer tick is local-only until `Finish` or manual `Save manual` writes a check-in. Ask the user to enter the observed minutes manually and save; diagnose persistence by checking the owner-scoped `habit_check_ins` row for that date.
