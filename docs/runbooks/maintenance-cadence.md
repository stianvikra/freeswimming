...
| 2026-06-19 | `tighten` | JS transfer default  | `390kb` -> `380kb` | `npm run test:perf:trend` recommended `tighten` with `10` weekly green runs and `17.8%` worst margin; used another conservative `10kb` step to stay just above the `15%` practical headroom rule.                |
| 2026-06-25 | `tighten` | CSS transfer default | `160kb` -> `150kb` | `npm run test:perf:trend` recommended `tighten` with `11` weekly green runs and `15.4%` worst margin; JS was too close for another `10kb` step, so this slice tightened CSS by one conservative `10kb` step.     |
