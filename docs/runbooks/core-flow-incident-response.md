...
- `/my-library/generator`
  ...
  - confirm `/my-library/workouts` stays a list-first browse surface with `My Swim Sessions`, `Build manual session`, and `AI-generated session` exposed as the three primary actions and no hidden editor mutation happening in the background,
  - confirm `Build manual session` creates a fresh canonical workout row directly and lands on the new session detail route with `Session details` expanded on first load,
