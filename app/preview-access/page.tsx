...
function getAdminUnlockPath(nextPath: string) {
  const params = new URLSearchParams({ next: nextPath });
  return `/preview-access/admin-unlock?${params.toString()}`;
}
...
export default async function PreviewAccessPage({ searchParams }: Props) {
  ...
  if (supabase && user) {
    ...
    if (role === "admin") {
      redirect(getAdminUnlockPath(nextPath));
    }
  }
      ...
      >
        <div className="pt-1">
          ...
          <div className="mt-5 max-w-[34rem]">
            ...
            <h1 className="mt-2 max-w-[12ch] text-[30px] leading-[1.02] font-semibold text-slate-900 sm:text-[40px]">
              Adult learner?
            </h1>
